import { decodeForwardSamples } from '$lib/media/demux/mp4';
import type {
	ClipAsset,
	DecodedFrameLike,
	EncodedVideoSample,
...
export type ClipDecoderState = 'idle' | 'configured' | 'decoding' | 'closed' | 'error';
...
export interface ClipDecoderOptions<Frame extends DecodedFrameLike> {
	...
	onStateChange?: (state: ClipDecoderState, queueSize: number | null) => void;
...
export class WebCodecsClipDecoder<Frame extends DecodedFrameLike> {
	...
	async decodeForward(asset: ClipAsset, timestampUs: number): Promise<DecodeForwardResult> {
		...
		try {
			...
			const cancelled = outcome === 'cancel' || generation !== this.generation;
	...
	private acceptOutput(frame: Frame, clipId: string, generation: number) {
		...
		try {
			...
			frame.close();
			this.failActive(error instanceof Error ? error : new Error(String(error)), generation);
		}
	}
	...
	private reportState() {
		this.options.onStateChange?.(this.state, this.active?.decodeQueueSize ?? null);
	}
}
