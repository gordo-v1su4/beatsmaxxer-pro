...
export function createBrowserVideoDecoderFactory<Frame extends DecodedFrameLike>(
	createFrame: (frame: VideoFrame) => Frame
): VideoDecoderFactory<Frame> {
	return {
		create(callbacks) {
		...
		typeof VideoDecoder !== 'undefined'
			? {
					isConfigSupported(config: VideoDecoderConfigLike) {
						return VideoDecoder.isConfigSupported(config as VideoDecoderConfig);
					}
				}
			: null;
...
export async function probeBrowserSampleFrame(config: VideoDecoderConfigLike) {
