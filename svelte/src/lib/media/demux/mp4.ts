import { createClipAsset } from '$lib/media/ClipAsset';
import type { ClipAsset, DemuxedVideoTrack, EncodedVideoSample } from '$lib/media/types';
...
export interface Mp4DemuxAdapter {
	readonly id: string;
	demux(source: ArrayBuffer, signal?: AbortSignal): Promise<DemuxedVideoTrack>;
}

export class Mp4DemuxBoundary {
	constructor(private readonly adapter: Mp4DemuxAdapter | null = null) {}

	async demux(id: string, source: ArrayBuffer, signal?: AbortSignal): Promise<ClipAsset> {
...
export function sampleAtTimestamp(asset: ClipAsset, timestampUs: number): EncodedVideoSample {
	const clamped = Math.max(
		asset.startTimestampUs,
		Math.min(timestampUs, asset.startTimestampUs + asset.durationUs - 1)
	);
	let selected = asset.samples[0];
	let selectedTimestamp = Number.NEGATIVE_INFINITY;
	for (const sample of asset.samples) {
		if (sample.timestampUs <= clamped && sample.timestampUs > selectedTimestamp) {
			selected = sample;
