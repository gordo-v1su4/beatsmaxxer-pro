...
export function estimatedResidentBytes(clip:Pick<Clip,'width'|'height'|'duration'|'fps'>){
  if(![clip.width,clip.height,clip.duration,clip.fps].every(value=>Number.isFinite(value)&&value>0))throw new Error('Invalid resident clip metadata');
  return clip.width*clip.height*4*Math.ceil(clip.duration*clip.fps);
}
...
export class GpuBankDeck implements DeckAdapter {
  ...
  async load(){
    if(!Number.isFinite(this.budget)||this.budget<=0)throw new Error('Invalid resident memory cap');
    ...
    }catch(error){this.frames.forEach(frame=>frame.texture.destroy());this.frames=[];this.bytes=0;throw error;}
