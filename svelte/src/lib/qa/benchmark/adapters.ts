...
export type Backend='beatsmaxxer'|'mediabunny'|'gpu-bank';
export function makeAdapter(backend:Backend,clip:Clip,onFrame:(o:Observation)=>void,onError:(e:string)=>void,budget:number):DeckAdapter {
  ...
  throw new Error('Resident banks require the shared GPU device');
}
class HtmlDeck implements DeckAdapter {
  ...
  target(t:Target,_future:Cut[]){
    ...
    const timeline: TimelineFrame = {
      generation:t.epoch, playing:t.playing, contextTimeSeconds:t.time,
      positionSeconds:t.time, transportSeconds:t.time, playbackRate:1,
      frameId:0, audioFrameId:0, reason:'initial', bpm:120,
      fixedStepSeconds:1/60, fixedStepIndex:0, fixedStepPhase:0,
      deterministicSeed:0, events:[], beatPosition:0, beatPhase:0,
      beatIndex:0, beatIntervalSeconds:.5, source:'hosted-grid', fallbackReason:null
    };
    this.pool.syncControlledModule('deck',t.source,1,timeline,t.event.id);
  }
