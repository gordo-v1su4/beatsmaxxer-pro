...
export class FrameScore {
  ...
  observe(time:number,generation:number,pts:number,frameDuration:number,displayInterval=1/60):boolean {
    ...
    const accepted=Number.isFinite(error)&&generation===target.event.id&&error<=displayInterval&&error>=-frameDuration-displayInterval;
