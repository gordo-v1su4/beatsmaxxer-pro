...
import {evaluateAcceptance,scoreProgramCuts} from './acceptance';
...
let fixtureProvenance:Record<string,unknown>={};
const displayInterval=()=>percentile(frameTimes.filter(t=>t>0&&t<.1).slice(-120),.5)??1/60;
Object.assign(window, {__BMX_BENCHMARK__: {
  snapshot:()=>({playing,busy,status:status.textContent,records}),
}});
...
async function init(){
  fixtureProvenance=await (await fetch('/benchmark-fixtures/provenance.json')).json();
  manifest=await (await fetch('/benchmark-fixtures/benchmark/manifest.json')).json();
  baseGrid=await (await fetch(manifest.grid.url)).json();
  originalGrid=baseGrid;
  const response=await fetch('/benchmark-fixtures/benchmark/audio-events.json');
  ...
  const midiBytes=await (await fetch('/benchmark-fixtures/benchmark/redline-midi.json')).arrayBuffer();
  ...
  const triggerBytes=await (await fetch('/benchmark-fixtures/benchmark/redline-analysis.json')).arrayBuffer();
  ...
  const interpolation=await fetch('/benchmark-fixtures/benchmark/interpolation-manifest.json');
...
async function begin(){
  ...
  try{
    ...
    currentPresenter.device.lost.then(info=>{
      if(presenter===currentPresenter&&(playing||busy)){
        invalid.push(`GPU device lost: ${info.message}`);suiteCancelled=true;
        if(playing)void finish();
      }
    });
    currentPresenter.device.addEventListener('uncapturederror',(event)=>{
      if(presenter===currentPresenter){invalid.push(event.error.message);suiteCancelled=true;}
    });
    ...
    adapters=clips.map((c,i)=>{
      ...
      if(o.generation!==target.event.id||error>displayInterval()||error< -o.duration-displayInterval()){scores[i].observe(now,o.generation,o.pts,o.duration,displayInterval());return;}
    ...
    const loads=await Promise.allSettled(adapters.map(a=>a.load()));
    const failed=loads.find(result=>result.status==='rejected');
    if(failed?.status==='rejected')throw failed.reason;
    if(id!==runId)return;
    if(document.hidden||invalid.length)throw new Error(invalid.join('; ')||'Tab hidden during preload');
    ...
    settings.schemaVersion=11;settings.application='beatsmaxxer-pro';settings.fixtureProvenance=fixtureProvenance;
    settings.baseline='Live $lib/media/VideoPool and $lib/timesampler/reducer';
    settings.gpu=Object.fromEntries(['vendor','architecture','device','description'].map(key=>[key,(currentPresenter.adapterInfo as unknown as Record<string,unknown>)[key]]));
    startAudio();status.textContent=`Playing ${clips.length} decks · ${backend}`;pause.disabled=false;raf=requestAnimationFrame(tick);
  }catch(e){if(id!==runId)return;suiteCancelled=true;await persist({kind:'load-failure',backend:select('backend').value,completed:false,invalid:[String(e)]});status.textContent=String(e);await cleanup();controls(false);play.disabled=false;finishRun?.();finishRun=null;}
...
function tick(now:number){
  ...
  for(let i=0;i<adapters.length;i++){
    ...
    const frame=fresh[i];
    if(frame){
      if(scores[i].observe(time,frame.generation,frame.pts,frame.duration,displayInterval())){
    ...
    }else{badDriftSince[i]??=0;longestBadDrift[i]=Math.max(longestBadDrift[i],time-badDriftSince[i]!);}
  ...
  if(beatIndex>lastProgramBeat){
    ...
    if(time>=programTimes[beatIndex]){
      programCuts.push({beat:beatIndex,scheduled:programTimes[beatIndex]??time,submitted:time,deck:pgm,ready:!!h&&h.generation===target.event.id&&error!==null&&error<=1/clips[pgm].fps+displayInterval(),sourceError:error});
...
async function finish(){
  ...
  const programSummary=scoreProgramCuts(programTimes,programCuts,offset,interval);
  const scored={...result,programSummary};
  await persist({...scored,acceptance:evaluateAcceptance(scored)});await cleanup();offset=0;controls(false);play.disabled=false;pause.disabled=true;
  status.textContent=`Run complete: ${programSummary.onTimeCuts}/${programSummary.cuts} program cuts within ${(interval*1000).toFixed(1)} ms (${programSummary.onTimePercent.toFixed(1)}%) · ${programSummary.lateCuts} late · ${programSummary.missedCuts} missing. Download results to save.`;finishRun?.();finishRun=null;
}
...
el('suite').onclick=async()=>{
  ...
  for(let trial=0;trial<3&&!suiteCancelled;trial++)for(const count of [1,4,6,8]){
    ...
    for(const backend of order){
      ...
      select('mode').value='cuts';select('trigger').value='legacy';modeControls();select('backend').value=backend;select('count').value=String(count);select('duration').value='120';select('pattern').value='audio-stutter4';select('resolution').value='720';select('view').value='switch';select('groove').value='straight';select('budget').value=backend==='gpu-bank'?'12288':'256';el<HTMLInputElement>('seed').value=String(42+trial);
