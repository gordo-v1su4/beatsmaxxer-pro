<script lang="ts">
  ...
  import { timingSettings, selectedTimingSlot, timingEditorCollapsed, timingEditorTab, timingLive, timingStatus, setClipTiming } from '$lib/stores/timing';
  ...
  function update(patch:Partial<ClipTiming>){setClipTiming($selectedTimingSlot,{...config,...patch});}
  ...
  {#if !$timingEditorCollapsed}
    ...
    <div class="editor-controls">
      ...
      {#if $timingEditorTab==='ramp'}
        <label>MIN × <input aria-label="Ramp minimum speed" type="number" min=".25" max="4" step=".05" value={Number(range.min.toFixed(2))} onchange={e=>{const n=Number(e.currentTarget.value);if(Number.isFinite(n))update({ramp:scaleRampRange(config.ramp,n,Math.max(n,range.max))});}}/></label>
        <label>MAX × <input aria-label="Ramp maximum speed" type="number" min=".25" max="4" step=".05" value={Number(range.max.toFixed(2))} onchange={e=>{const n=Number(e.currentTarget.value);if(Number.isFinite(n))update({ramp:scaleRampRange(config.ramp,Math.min(n,range.min),n)});}}/></label>
        <label>CYCLE <select aria-label="Ramp cycle" value={config.ramp.cycleBeats} onchange={(e)=>update({ramp:{...config.ramp,cycleBeats:Number(e.currentTarget.value)}})}>{#each [.5,1,2,4,8,12,16,20,24,28,32] as beats}<option value={beats}>{beats<4?`${beats} BEAT${beats===1?'':'S'}`:`${beats/4} BAR${beats===4?'':'S'}`}</option>{/each}</select></label>
        <label>SHAPE <select aria-label="Ramp interpolation" value={config.ramp.shape} onchange={(e)=>update({ramp:{...config.ramp,legacy:undefined,shape:e.currentTarget.value as CurveShape}})}>{#each ['smooth','sine','linear','tension','hold'] as shape}<option value={shape}>{shape.toUpperCase()}</option>{/each}</select></label>
        <label>SNAP <select aria-label="Ramp snapping" bind:value={snap}>{#each ['off','bar','beat','16th','32nd','64th'] as option}<option value={option}>{option.toUpperCase()}</option>{/each}</select></label>
        <button onclick={()=>update({ramp:defaultClipTiming().ramp})}>RESET SHAPE</button>
        <span class="hint">{config.ramp.points.length} POINTS · CLICK ADD · ALT = FREE MOVE · DOUBLE CLICK DELETE</span>
      {:else}
        ...
        <label>DIVISION <select aria-label="Stutter division" value={config.stutter.division} onchange={(e)=>update({stutter:{...config.stutter,division:Number(e.currentTarget.value)}})}>{#each [.125,.25,.5,1,2,4] as b}<option value={b}>{b===4?'1 BAR':`1/${4/b}`}</option>{/each}</select></label>
        ...
        {#if config.stutter.mode==='jump'}<label>SLICES <select aria-label="Jump slices" value={config.stutter.slices} onchange={(e)=>update({stutter:{...config.stutter,slices:Number(e.currentTarget.value)}})}>{#each [4,8,16,32,64] as n}<option>{n}</option>{/each}</select></label>{/if}
      {/if}
      </div>
    </div>
    <div class="effect-stage">
    {#if $timingEditorTab==='ramp'}
      <div class="shape-bank" aria-label="Ramp shape presets">{#each RAMP_PRESET_NAMES as name}<button title={name==='COSINE'?'ZigSwap cosine · 0.5× → 2× → 0.5×':name==='SMASH'?'ZigSwap smash · 12 beats at ¼×, 1 at 4×, 3 at 1×':`Perform ${name} curve`} aria-label={`Ramp shape ${name}`} onclick={()=>update({ramp:rampPreset(name,config.ramp.cycleBeats)})}><svg viewBox="0 0 64 24" aria-hidden="true"><path d={RAMP_PREVIEW_PATHS[name]} fill="none" stroke="currentColor" stroke-width="1"/></svg><span>{name}</span></button>{/each}</div>
      <TimingCurve ramp={config.ramp} phase={config.effect==='ramp'?live?.phase??0:0} {snap} onchange={(ramp)=>update({ramp})}/>
    {:else}
      <div class="shape-bank-spacer" aria-hidden="true"></div>
...
<style>
  .effect-stage{height:calc(74px + var(--timing-plot-height,200px));min-width:0;background:#090b0c}
  .shape-bank-spacer{height:48px}
  .effect-parameters{display:flex;flex-wrap:wrap;align-items:center;gap:8px 12px;min-width:0}
