  ...
  if (type === 'shaper') {
    ...
    void main(){
      ...
      if(uMode < 0.5){
        ...
        float curve = smoothstep(0.0,0.9,uBeatPhase) - smoothstep(0.85,1.0,uBeatPhase) * 3.0;
        float ramp = clamp(curve, -0.2, 1.0);
        float warp = sin((uv.y + offset*0.5) * (4.0 + freq*10.0) + t * (1.0 + freq*2.0)) * (amount*0.08 + low*0.06);
        warp = clamp(warp, -(0.01 + clip*0.07), 0.01 + clip*0.07);
        vec2 st = uv;
        st.x += ramp * amount * 0.12 + warp;
        st.y += pulse * 0.05 * sin(uv.x * 10.0 + t*2.0);
        vec3 wet = sampleVideo(st);
        wet = mix(wet, wet * uColor * 2.0, amount * 0.28);
        wet *= 1.0 + pulse * 0.18;
