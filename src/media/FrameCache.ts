...
export class FrameLease<Frame extends DecodedFrameLike> {
  ...
  clone(owner: string): DetachedFrameLease<Frame> {
    ...
    return new DetachedFrameLease(
      owner,
      this.token.frame.clone() as Frame,
    );
  }
...
export class PresentationReceipt<Frame extends DecodedFrameLike> {
  ...
  static submitted<Frame extends DecodedFrameLike>(
...
export class FrameCache<Frame extends DecodedFrameLike> {
  ...
  acquireForTimestamp(
    ...
    for (const entry of this.entries.values()) {
      ...
      if (
        selected === null ||
        entry.timestampUs > selected.timestampUs
      ) {
        selected = entry;
      }
      if (
  ...
  clearGeneration(clipId: string, generation: number) {
    this.removeMatching(
      (entry) =>
        entry.clipId === clipId && entry.generation === generation,
    );
  }
  ...
  private removeMatching(
    predicate: (entry: FrameEntry<Frame>) => boolean,
  ) {
